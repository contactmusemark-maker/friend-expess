...
import { Wallet, Mail, User, Loader2, ArrowRight, CheckCircle2 } from 'lucide-react'
    ...
    <div className="animate-fade-in-up">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {step === 'form' ? 'Start tracking expenses with friends' : `We sent a 6-digit code to ${email}`}
        </p>
      </div>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                ...
                required
                autoFocus
                className="w-full bg-secondary/50 border border-border rounded-lg px-4 py-3 text-2xl text-center tracking-[0.5em] font-bold text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/40 transition-all"
              />
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-background font-semibold py-2.5 rounded-lg transition-all"
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Create account <CheckCircle2 className="w-4 h-4" /></>}
            </button>
